...
import { User, AdminSection, Class, Venue, Registration, AppSettings, ChatMessage, Instructor } from '../types';
...
const TrainerSidebar: React.FC<TrainerSidebarProps> = ({ user, onSignOut, activeSection, onSectionChange, isOpen, onClose }) => {
    const navItems = [
                ...
                <div className="pt-6 border-t border-[#FBF7EF]/10">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-[#FBF7EF]/60">Trainer</p>
                        </div>
                    </div>
...
//=============== Trainer My Classes Screen ===============//
interface TrainerMyClassesProps {
    classes: Class[];
...
const TrainerMyClasses: React.FC<TrainerMyClassesProps> = ({ classes, registrations, venues, instructors, onShowToast }) => {
    // Filter classes - for demo, show all upcoming published classes (in real app, would filter by assigned instructor)
...
const TrainerSchedule: React.FC<TrainerScheduleProps> = ({ classes }) => {
            ...
            <div className="bg-white rounded-[2rem] p-6 md:p-8 border border-[#6E7568]/10 shadow-sm">
                ...
                <div className="grid grid-cols-7 gap-2 md:gap-4">
                    ...
                    {days.map(day => {
                        ...
                        return (
                            <div key={day} className={`aspect-square rounded-xl md:rounded-2xl border ${isToday ? 'border-[#6E7568] bg-[#FBF7EF] shadow-inner' : 'border-[#6E7568]/10 hover:border-[#6E7568]/30'} p-1 md:p-2 relative transition-all duration-300`}>
...
//=============== Trainer Attendees Screen ===============//
interface TrainerAttendeesProps {
    classes: Class[];
    registrations: Registration[];
}
const TrainerAttendees: React.FC<TrainerAttendeesProps> = ({ classes, registrations }) => {
    const [filterClass, setFilterClass] = useState("all");
    
    // Filter registrations to only show attendees for trainer's classes
    const trainerClassIds = classes
        .filter(c => c.status === 'published')
        .map(c => c.id);
    
    const filtered = registrations.filter(r => 
        trainerClassIds.includes(r.classId) && 
        ...
        <div className="animate-fade-in">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 border-b border-[#26150B]/5 pb-4">
                ...
                >
                    ...
                    {classes.filter(c => c.status === 'published').map(c => (
...
const TrainerProfile: React.FC<TrainerProfileProps> = ({ user, onUpdateUser }) => {
        ...
        <div className="animate-fade-in space-y-8">
            ...
            <div className="bg-white rounded-[2rem] p-8 border border-[#6E7568]/10 shadow-sm">
                <div className="flex items-center gap-6 mb-8">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-[#6E7568]">Trainer</p>
            ...
            <div className="bg-white rounded-[2rem] p-6 border border-[#6E7568]/10 shadow-sm">
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between p-3 bg-red-50 rounded-xl">
                        <span className="text-sm text-[#26150B]">Manage other trainers</span>
                        <Icons.XIcon size={18} className="text-red-400" />
                    </div>
                </div>
            </div>
...
export const TrainerApp: React.FC<TrainerAppProps> = ({
    user,
    onSignOut,
    classes,
    registrations,
    ...
    return (
        <div className="flex h-screen bg-[#FBF7EF] overflow-hidden font-['Montserrat']">
            ...
            <main className="flex-1 flex flex-col h-full relative overflow-hidden">
                {/* Mobile Header */}
                <div className="lg:hidden p-4 flex items-center justify-between border-b border-[#6E7568]/10 bg-[#FBF7EF] relative z-20">
                    <PauseLogo size="sm" />
                    <button onClick={() => setSidebarOpen(true)} className="text-[#6E7568]">
                        <Icons.MenuIcon />
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-8 custom-scrollbar relative">
                    ...
                    <div className="max-w-6xl mx-auto relative z-10">
                        ...
                        {activeSection === 'schedule' && (
                            <TrainerSchedule classes={classes} />
                        )}
                        {activeSection === 'attendees' && (
                            <TrainerAttendees classes={classes} registrations={registrations} />
                        )}
                        ...
                        {activeSection === 'profile' && (
                            <TrainerProfile user={user} onUpdateUser={onUpdateUser} />
                        )}
                    </div>
                </div>
            </main>
